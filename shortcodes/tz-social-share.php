...
class TZ_Share_Buttons{
	...
	public function getAll() {
		...
		return '<div class="social-links"><span class="shares">'.__('Shares: ', 'themeszone').'</span>'.implode('', $button_array).'</div>';
	}
	...
	private function buildSocialButton($this_one) {
		...
		return '<div class="tz-post-share" data-service="'.esc_attr($this_one).'" data-postID="'.get_the_ID().'">
	...
	private function getSocialUrl($service) {
		...
		if (class_exists('Woocommerce') && is_product()) {
			$text = urlencode( __("A great product: ", 'themeszone').$post->post_title );
		} else {
			$text = urlencode( __("A great post: ", 'themeszone').$post->post_title);
		}
		...
		switch ($service) {
			...
			case "facebook" :
				$api_link = 'https://www.facebook.com/sharer/sharer.php?s=100&amp;p%5Btitle%5D='.urlencode($post->post_title).'&amp;p%5Bsummary%5D='.urlencode($short_description).'&amp;p%5Burl%5D='.$escaped_url.'&amp;p%5Bimages%5D%5B0%5D='.urlencode($image[0]);
				break;
			...
			case "pinterest" :
				if (isset($image)) {
					$api_link = 'http://pinterest.com/pin/create/bookmarklet/?media='.$image[0].'&amp;url='.$escaped_url.'&amp;title='.esc_attr(preg_replace('/\s+/', '+', get_the_title())).'&amp;description='.esc_html( $post->post_excerpt );
				}
				else {
			...
			case "mail" :
				$subject = __('Check this!', 'themeszone');
				$body = __('See more at: ', 'themeszone');
...
function tz_share_buttons_output() {
	$buttons = new TZ_Share_Buttons();
	$out = $buttons->getAll();
	echo $out;
}

add_shortcode( 'tz_share_button', 'tz_share_buttons_output' );

/* Enqueue scripts */
function tz_share_scripts() {

	if ( is_single() && ( get_theme_mod('post_page_entry_footer', 1) == 1 ) ) {

		wp_enqueue_script( 'tz_share_post', plugin_dir_url(__DIR__) . '/assets/js/post-share.js', array('jquery'), '1.0', true);
		wp_localize_script( 'tz_share_post', 'ajax_var', array(
...
add_action( 'wp_enqueue_scripts', 'tz_share_scripts' );

/* Share post counters */
add_action( 'wp_ajax_nopriv_pt_post_share_count', 'tz_post_share_count' );
add_action( 'wp_ajax_pt_post_share_count', 'tz_post_share_count' );
...
function tz_post_share_count() {
	...
	if ( isset( $_POST['tz_post_share_count'] ) ) {
