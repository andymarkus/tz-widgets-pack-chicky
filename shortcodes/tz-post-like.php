...
function tz_like_scripts() {
	...
	if ( ( get_theme_mod('post_page_entry_footer', 1) == 1 ) ) {

		wp_enqueue_script( 'tz-like-post', plugin_dir_url(__DIR__) . '/assets/js/post-like.js', array('jquery'), '1.0', true );
		wp_localize_script( 'tz-like-post', 'ajax_var', array(
...
add_action( 'wp_enqueue_scripts', 'tz_like_scripts' );
...
function tz_output_like_button( $post_id ) {
	...
	<div class="like-wrapper">
		<span><?php _e('Likes: ', 'themeszone'); ?></span>
		<a href="#" class="tz-post-like<?php echo esc_attr($class);?>" data-post_id="<?php echo esc_attr($post_id); ?>" title="<?php echo esc_attr($title); ?>">
 ...
 */
function tz_output_likes_counter($post_id) {
...
function tz_show_like(){
	global $post;
	tz_output_like_button( $post->ID );
	tz_output_likes_counter( $post->ID );
}

add_shortcode( 'tz_likes', 'tz_show_like' );
